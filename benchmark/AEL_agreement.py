...
for index in range(0,Zookeeper_num-1,1):
    ...
    for para_index in range(0,n_para-1,1):
        para_info = str(ec[para_index]) + ',' + str(mp[para_index])
        print(para_info)
        parser_1 = AEL.LogParser(log_format=setting['log_format'], indir=indir, outdir=output_dir_1,
                             minEventCount=ec[para_index], merge_percent=mp[para_index], rex=setting['regex'])
        parser_2 = AEL.LogParser(log_format=setting['log_format'], indir=indir, outdir=output_dir_2,
                             minEventCount=ec[para_index], merge_percent=mp[para_index], rex=setting['regex'])
...
print(df_result)
df_result.to_csv('AEL_agreement_Zookeeper.csv')
