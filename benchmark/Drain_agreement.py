...
st = [0.2, 0.5, 0.2, 0.39, 0.7, 0.5, 0.6, 0.6]
d = [6, 4, 4, 6, 6, 5, 5, 3]
n_para = 8
...
for index in range(0,Zookeeper_num-1,1):
    ...
    for para_index in range(0,n_para-1,1):
        para_info = str(st[para_index]) + ',' + str(d[para_index])
        print(para_info)
        parser_1 = Drain.LogParser(log_format=setting['log_format'], indir=indir, outdir=output_dir_1, rex=setting['regex'], depth=d[para_index], st=st[para_index])
        parser_2 = Drain.LogParser(log_format=setting['log_format'], indir=indir, outdir=output_dir_2, rex=setting['regex'], depth=d[para_index], st=st[para_index])
...
print(df_result)
df_result.to_csv('Drain_agreement_Zookeeper.csv')
