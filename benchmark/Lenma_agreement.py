...
threshold = [0.86, 0.91, 0.7, 0.9, 0.5, 0.8, 0.88, 1, 0.6, 0.78]
n_para = 10
...
setting = benchmark_settings['Windows']

agreement_result = []
for index in range(0,Windows_num,1):
    logfile_1 = Windows_file + '.part' + str(index)
    logfile_2 = Windows_file + '.part' + str(index+1)
    indir = input_dir + Windows_dir

    for para_index in range(0,n_para-1,1):
        para_info = str(threshold[para_index])
        print(para_info)
        parser_1 = LenMa.LogParser(log_format=setting['log_format'], indir=indir, outdir=output_dir_1, rex=setting['regex'], threshold=threshold[para_index])
        parser_2 = LenMa.LogParser(log_format=setting['log_format'], indir=indir, outdir=output_dir_2, rex=setting['regex'], threshold=threshold[para_index])
...
print(df_result)
df_result.to_csv('LenMa_agreement_Windows.csv')
