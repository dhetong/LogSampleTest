...
for index in range(0,Zookeeper_num,1):
    ...
    m_percent = 0.4
    while m_percent <= 0.7:
        for e_count in eventcount_group:
            para_info = str(m_percent) + ',' + str(e_count)
            print(para_info)
	    parser = AEL.LogParser(log_format=setting['log_format'], indir=indir, outdir=output_dir,
                             minEventCount=e_count, merge_percent=m_percent, rex=setting['regex'])
