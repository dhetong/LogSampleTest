...
for index in range(0,Zookeeper_num,1):
    ...
    while tau <= 0.95:
        para_info = str(tau)
        print(para_info)
	parser = Spell.LogParser(log_format=setting['log_format'], indir=indir, outdir=output_dir, rex=setting['regex'], tau=tau)
...
print(df_result)
df_result.T.to_csv('Spell_sample_result.csv')
